...
import { uploadBuffer, presignedUrl } from "../s3.js";
...
router.get("/", authMiddleware, async (req, res) => {
  const r = await db.query(
    "SELECT id, filename, size, mime, tags, created_at FROM files WHERE user_id=$1 ORDER BY created_at DESC",
    [req.user.id]
  );
  res.json(r.rows);
});
...
router.delete("/:id", authMiddleware, async (req, res) => {
  await db.query("DELETE FROM files WHERE id=$1 AND user_id=$2", [
    req.params.id,
    req.user.id
  ]);
  res.json({ ok: true });
